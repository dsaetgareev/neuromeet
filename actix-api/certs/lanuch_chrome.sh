...
SPKI=`openssl x509 -inform der -in fullchain.der -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64`
...
case `uname` in
    ...
    (*Linux*)  google-chrome --origin-to-force-quic-on=85.208.87.7:4433 --ignore-certificate-errors-spki-list=$SPKI --enable-logging --v=1 ;;
